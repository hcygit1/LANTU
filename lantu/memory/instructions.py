...
def load_instructions(project_root: str) -> str:
    ...
    发现顺序（低优先级在前，高优先级在后）：
    1. 用户全局：~/.lantu/LANTU.md, ~/.lantu/AGENTS.md
    2. 项目目录链：从 git root 到 workDir，每个目录的 LANTU.md 和 AGENTS.md
    3. workDir/.lantu/INSTRUCTIONS.md（遗留格式）
    4. workDir/LANTU.local.md（本地覆盖）
    """
    ...
    # 1. 用户全局
    _add(home / ".lantu" / "LANTU.md")
    _add(home / ".lantu" / "AGENTS.md")
    ...
    # 3. 遗留格式
    _add(root / ".lantu" / "INSTRUCTIONS.md")

    # 4. 本地覆盖
    _add(root / "LANTU.local.md")
