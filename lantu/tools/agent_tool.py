...
class AgentTool(Tool):
    ...
    async def execute(self, params: BaseModel) -> ToolResult:
        ...
        from lantu.agents.fork import ForkError, build_forked_messages
        from lantu.agents.parser import AgentDef
        from lantu.agents.tool_filter import clone_registry_for_fork, resolve_agent_tools
        from lantu.agent import Agent as AgentClass
        ...
        if p.subagent_type is None:
            from lantu.context import clone_replacement_state
    ...
    async def _execute_as_teammate(self, p: AgentToolParams) -> ToolResult:
        ...
        from lantu.agents.fork import ForkError, build_forked_messages
        from lantu.agents.parser import AgentDef
        from lantu.agents.tool_filter import build_teammate_tools
        from lantu.agent import Agent as AgentClass
        from lantu.conversation import ConversationManager
        from lantu.permissions import (
            DangerousCommandDetector,
        ...
        from lantu.teams.models import BackendType, TeammateInfo
    ...
    def _spawn_pane_teammate(
        ...
        try:
            if backend == BackendType.TMUX:
                from lantu.teams.spawn_tmux import spawn_tmux_teammate
                ...
                from lantu.teams.spawn_iterm2 import spawn_iterm2_teammate
    ...
    async def _execute_with_worktree(self, p: AgentToolParams) -> ToolResult:
        ...
        from lantu.worktree.integration import (
