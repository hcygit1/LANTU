...
def _session_path(lantu_dir: Path) -> Path:
    return lantu_dir / SESSION_FILENAME
...
def load_worktree_session(lantu_dir: Path) -> WorktreeSession | None:
    path = _session_path(lantu_dir)
    if not path.exists():
        return None
    try:
