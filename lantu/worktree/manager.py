...
from lantu.worktree.changes import (
    CleanupResult,
    Changes,
...
from lantu.worktree.setup import perform_post_creation_setup
...
class WorktreeManager:
    def __init__(
        ...
        self.worktree_dir = worktree_dir or str(
            Path(repo_root) / ".lantu" / "worktrees"
        )
        self._lantu_dir = Path(repo_root) / ".lantu"
    ...
    async def enter(self, name: str) -> WorktreeSession:
        ...
        save_worktree_session(self._lantu_dir, session)
        return session
    ...
    async def exit(
        ...
        save_worktree_session(self._lantu_dir, None)
    ...
    def restore_session(self) -> WorktreeSession | None:
        session = load_worktree_session(self._lantu_dir)
        if session is None:
            return None
