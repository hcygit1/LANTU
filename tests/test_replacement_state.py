...
"""ContentReplacementState 的测试 —— 方案 A（决策冻结，原地修改）。"""
...
def test_apply_mutates_conv_in_place(tmp_path: Path) -> None:
    ...
    # 原始 conv 被原地替换（方案 A 的不变量）
    assert id(conv.history) == orig_history_id
    content = conv.history[0].tool_results[0].content
    assert content.startswith(PERSISTED_TAG)
    assert records[0].replacement == content
